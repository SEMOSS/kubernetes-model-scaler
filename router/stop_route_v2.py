        ...
        try:
            ...
            try:
                ...
                if deployer.restore_original_state():
                    logger.info(
                        f"Successfully restored model {model_id} to original state"
                    )
                else:
                    logger.error(
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to stop model: {str(e)}"
            )

    return await asyncio.to_thread(blocking_removal)
